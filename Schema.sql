...
('Drifting School', 'A', '2020-11-25', '10:30:00', '12:30:00'),
('Old Timer Race', 'B', '2020-11-25', '14:30:00', '15:00:00'),
('TwoWeels Showoff', 'A', '2020-11-30', '12:00:00', '15:00:00'),
('Retro Showoff', 'A', '2020-12-02', '10:00:00', '13:20:00');
...
-- Query:
	-- Selezione di tutti i vigilanti che hanno fatto esclusivamente turni notturni
	-- Lista degli espositori che esponevano almeno 5 veicoli il cui valore totale supera i 200.000 ma non ne hanno venduto nessuno
	-- Lista di tutte le clienti donne con una mail di outlook che hanno acquistato almeno 2 veicoli per un importo totale inferiore a 100.000
	-- Calcolare il profitto totale ottenuto dalla vendita dei veicoli ragguppati per marca e per i quali sono stati venduti almeno due veicoli il cui importo totale sia almeno 75000
	-- Tipo di biglietto più venduto tra gli under 18 e il suo profitto totale nella prima settimana dell'evento
...
SELECT CF, COUNT(Targa) AS VeicoliMercedes 
FROM Espositore E JOIN Veicolo V ON E.CF = V.Espositore
WHERE V.Marca = 'Mercedes'
GROUP BY CF
HAVING COUNT(Targa) >= 5;

SELECT CF,COUNT(CF) AS VeicoliComprati, Cognome, Nome, SUM(Prezzo) AS ImportoTotale FROM Cliente C JOIN Veicolo V on C.CF = V.Acquirente
WHERE Mail like '%@outlook.com' AND Sesso = 'F'
GROUP BY C.CF
HAVING COUNT(CF) >= 2 AND SUM(Prezzo) <= 100000;

SELECT Marca, COUNT(Targa) AS NumeroVeicoliVenduti, SUM(Prezzo) FROM Veicolo
WHERE Prezzo > 0
GROUP BY Marca
HAVING COUNT(Targa)>= 2 AND SUM(Prezzo) >= 75000;

SELECT Tipo, COUNT(CF) AS NumeroBiglietti FROM Cliente C JOIN Biglietto B on C.Biglietto =  B.Numero
WHERE C.CF >= '______02________'
GROUP BY Tipo;
--HAVING COUNT(CF) >
